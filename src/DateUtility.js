class Utility {
  ...
  calculateDailyAverge(id, relevantData) { // returns relevantData(sleepQUality etc. daily average for users data)
    let perDayAmount = this.dataSet.filter((data) => id === data.userID);
    return perDayAmount.reduce((sumSoFar, data) => {
      return sumSoFar += this.dataSet[relevantData];
    }, 0) / perDayAmount.length;
  }

  displayDailyData(id, date, relevantData) { // returns single datapoint for a user on a day
    let findDailyData = this.dataSet.find((data) => id === data.userID && date === data.date);
    return findDailyData.relevantData;
  }

  calculateWeeklyAverage(date, id, userRepo) {
